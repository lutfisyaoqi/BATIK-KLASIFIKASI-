...
export default function Admin() {
  ...
  const verificationAttemptRef = useRef(false);

  // Token verification on mount
  useEffect(() => {
    if (verificationAttemptRef.current) return;
  ...
  }, [historyRefreshTrigger, isAuthenticated]);
  ...
  }, [dashboardRefreshTrigger, isAuthenticated]);
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case 'training':
        return <TrainingPanel onTrainingSuccess={handleTrainingSuccess} />;
      case 'history':
        return (
          <HistoryTable
            items={history}

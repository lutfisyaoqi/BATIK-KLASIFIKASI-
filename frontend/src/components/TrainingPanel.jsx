...
export default function TrainingPanel({ onTrainingSuccess }) {
  ...
  const waitForCompletion = async () => {
    const MAX_ATTEMPTS = 200; // sekitar 10 menit
    let attempt = 0;
    ...
    throw new Error('Training polling timed out. Pastikan backend masih berjalan.');
  };
  ...
  const handleTrain = async () => {
    ...
    try {
      ...
      setMessage(data.message || 'Training dimulai. Menunggu proses selesai...');

      const finalData = await waitForCompletion();
      if (finalData && finalData.state === 'completed') {
        setMessage('Training selesai. Model siap digunakan.');
        setStatus('Training selesai.');
        setProgress(100);
        if (onTrainingSuccess) onTrainingSuccess();
      } else {
        setError('Training tidak selesai dengan benar. Silakan cek status backend.');
        setMessage('Training berhenti sebelum selesai.');
        setStatus(finalData?.state || 'error');
      }
    } catch (err) {

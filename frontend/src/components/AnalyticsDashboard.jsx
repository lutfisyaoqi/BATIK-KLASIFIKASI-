...
export default function AnalyticsDashboard({ refreshTrigger, hasHistory }) {
  ...
  const loadAnalytics = async () => {
    try {
      ...
      const [analyticsRes, historyRes, evalRes, metricsRes] = await Promise.all([
        api.get('/ml/analytics'),
        api.get('/ml/training-history'),
        api.get('/ml/evaluation'),
        api.get('/ml/model-metrics'),
      ]);
    ...
    <div className="space-y-8">
      ...
      {modelMetrics && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="card-shell surface p-6">
            <div className="flex items-center gap-4">
              <div className="p-3 rounded-2xl bg-emerald-100 text-emerald-700 shadow-sm">📈</div>
              <div>
                <p className="text-sm font-medium text-slate-500">Model Accuracy</p>
                <p className="text-2xl font-semibold text-slate-950">{(modelMetrics.accuracy * 100).toFixed(1)}%</p>
              </div>
            </div>
          </div>
          ...
          <div className="card-shell surface p-6">
            <div className="flex items-center gap-4">
              <div className="p-3 rounded-2xl bg-blue-100 text-blue-700 shadow-sm">🎯</div>
              <div>
                <p className="text-sm font-medium text-slate-500">Precision</p>
                <p className="text-2xl font-semibold text-slate-950">{isNaN(modelMetrics.precision) ? '—' : (modelMetrics.precision * 100).toFixed(1) + '%'}</p>
              </div>
            </div>
          </div>
          ...
          <div className="card-shell surface p-6">
            <div className="flex items-center gap-4">
              <div className="p-3 rounded-2xl bg-amber-100 text-amber-700 shadow-sm">🔍</div>
              <div>
                <p className="text-sm font-medium text-slate-500">Recall</p>
                <p className="text-2xl font-semibold text-slate-950">{isNaN(modelMetrics.recall) ? '—' : (modelMetrics.recall * 100).toFixed(1) + '%'}</p>
              </div>
            </div>
          </div>
          ...
          <div className="card-shell surface p-6">
            <div className="flex items-center gap-4">
              <div className="p-3 rounded-2xl bg-purple-100 text-purple-700 shadow-sm">⚡</div>
              <div>
                <p className="text-sm font-medium text-slate-500">F1 Score</p>
                <p className="text-2xl font-semibold text-slate-950">{isNaN(modelMetrics.f1_score) ? '—' : (modelMetrics.f1_score * 100).toFixed(1) + '%'}</p>
              </div>
            </div>
          </div>
        </div>
